...
func getMarshaledValidateCredReq(t *testing.T, token *auth.Token, verifier *auth.Token) []byte {
	req := &auth.ValidateCredentialReq{
...
func expectValidateResp(t *testing.T, respBytes []byte, expResp *auth.ValidateCredentialResp) {
	if respBytes == nil {
		t.Error("Expected non-nil response")
	}

	resp := &auth.ValidateCredentialResp{}
...
func TestSrvSecurityModule_ValidateCred_NoCred(t *testing.T) {
	...
	mod := NewSecurityModule(log, insecureTransportConfig())
	reqBytes := marshal(t, &auth.ValidateCredentialReq{})
	...
	expectValidateResp(t, resp, &auth.ValidateCredentialResp{
